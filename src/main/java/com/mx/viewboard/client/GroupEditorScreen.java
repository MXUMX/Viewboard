...
public final class GroupEditorScreen extends Screen {
    ...
    protected void init() {
        ...
        this.membersList = this.addRenderableWidget(new MembersList(Minecraft.getInstance(), this.width, this.height, this.listTop, MEMBER_ROW_HEIGHT));
    ...
    public boolean keyPressed(KeyEvent event) {
        if (this.popupMode != PopupMode.NONE) {
            ...
            if (this.popupMode != PopupMode.DELETE && event.key() == 257) {
                this.confirmPopup();
                return true;
            }
        ...
        if (this.captureMode != CaptureMode.NONE) {
            ...
            InputConstants.Key inputKey = InputConstants.Type.KEYSYM.getOrCreate(event.key());
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (this.popupMode != PopupMode.NONE && !this.isInsidePopup(event.x(), event.y())) {
            return true;
        }
        ...
        if (this.selectedGroup() != null && this.isInsidePanel(event.x(), event.y())) {
            if (this.panelCloseButton.mouseClicked(event, doubleClick)
                || this.renameButton.mouseClicked(event, doubleClick)
                || this.triggerButton.mouseClicked(event, doubleClick)
                || this.deleteButton.mouseClicked(event, doubleClick)
                || this.assignButton.mouseClicked(event, doubleClick)
                || this.removeMemberButton.mouseClicked(event, doubleClick)
                || this.membersList.mouseClicked(event, doubleClick)) {
                return true;
            }
            return true;
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.focusMapping != null) {
            guiGraphics.centeredText(this.font, Component.translatable("viewboard.groups.subtitle_keybind", Component.translatable(this.focusMapping.getName())), this.width / 2, 18, COLOR_SUBTEXT);
        }
        ...
        if (this.selectedGroup() != null) {
            ...
            this.membersList.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.panelCloseButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.renameButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.triggerButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.deleteButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.assignButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.removeMemberButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
        }

        if (this.popupMode != PopupMode.NONE) {
            this.renderPopup(guiGraphics);
            this.popupNameBox.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.popupConfirmButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
            this.popupCancelButton.extractRenderState(guiGraphics, mouseX, mouseY, partialTick);
        }
    }

    private void renderPanel(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        KeybindGroupConfig group = this.selectedGroup();
        if (group == null) {
            return;
        }
        ...
        guiGraphics.text(this.font, truncate(group.name(), Math.max(30, titleRight - x - 12)), x + 10, y + 8, COLOR_TEXT, false);
        guiGraphics.text(this.font, Component.translatable("viewboard.groups.trigger_summary", this.triggerLabel(group)), x + 10, y + 42, COLOR_SUBTEXT, false);
        guiGraphics.text(this.font, Component.translatable("viewboard.groups.members_count", group.members().size()), x + 10, y + 84, COLOR_TEXT, false);

        if (group.members().isEmpty()) {
            guiGraphics.centeredText(this.font, Component.translatable("viewboard.groups.no_members"), x + this.panelWidth / 2, y + 142, COLOR_SUBTEXT);
        }
    }

    private void renderPopup(GuiGraphicsExtractor guiGraphics) {
        ...
        guiGraphics.centeredText(this.font, title, x + w / 2, y + 10, COLOR_TEXT);
    ...
    private void applyLayoutToWidgets() {
        ...
        this.groupsList.updateSizeAndPosition(this.width, Math.max(40, this.listBottom - this.listTop), 0, this.listTop);
        ...
        this.membersList.updateSizeAndPosition(this.panelWidth, Math.max(40, this.panelY + this.panelHeight - this.memberListTop - 12), this.panelX, this.memberListTop);
    ...
    private void refreshAll() {
        ...
        if (this.membersList != null) {
            this.membersList.refreshEntries();
        }
        this.refreshButtons();
    }
    ...
    private void refreshButtons() {
        ...
        this.groupsList.active = !popupOpen;
        ...
        this.membersList.visible = panelOpen;
        this.membersList.active = panelOpen && !popupOpen;
    ...
    private final class GroupEntry extends ObjectSelectionList.Entry<GroupEntry> {
        ...
        @Override
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            if (focusMapping != null && this.quickWidth > 0 && this.isInsideQuickButton(event.x(), event.y())) {
        ...
        public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTick) {
            ...
            guiGraphics.text(font, truncate(this.group.name(), textRight - left - 12), left + 8, rowTop + 5, COLOR_TEXT, false);
            guiGraphics.text(
                font,
        ...
        private void renderQuickButton(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
            KeybindGroupConfig currentGroup = rules.groupFor(focusMapping).orElse(null);
    ...
    private final class MembersList extends ObjectSelectionList<MemberEntry> {
        private MembersList(Minecraft minecraft, int width, int height, int top, int itemHeight) {
            super(minecraft, width, height, top, itemHeight);
        }

        @Override
        public int getRowWidth() {
            return Math.max(40, this.width - 18);
        }
        ...
        private void refreshEntries() {
            this.clearEntries();
            KeybindGroupConfig group = selectedGroup();
            if (group == null) {
                return;
            }

            for (KeybindMemberConfig member : group.members()) {
                this.addEntry(new MemberEntry(member.keybindId()));
            }
        }
    }

    private final class MemberEntry extends ObjectSelectionList.Entry<MemberEntry> {
        private final String keybindId;

        private MemberEntry(String keybindId) {
            this.keybindId = keybindId;
        }

        @Override
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            selectedMemberKeybindId = this.keybindId.equals(selectedMemberKeybindId) ? null : this.keybindId;
            refreshButtons();
            return true;
        }
        ...
        @Override
        public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float partialTick) {
            boolean selected = this.keybindId.equals(selectedMemberKeybindId);
            int top = this.getY();
            int left = this.getX();
            int width = this.getWidth();
            int rowTop = top + 2;
            int rowHeight = MEMBER_ROW_HEIGHT - 4;
            guiGraphics.fill(left, rowTop, left + width, rowTop + rowHeight, selected ? COLOR_ACCENT : hovered ? 0x55363636 : 0x33242424);
            guiGraphics.text(font, truncate(mappingName(this.keybindId).getString(), width - 12), left + 6, rowTop + 6, COLOR_TEXT, false);
        }

        @Override
        public Component getNarration() {
            return mappingName(this.keybindId);
        }
    }
